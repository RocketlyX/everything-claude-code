---
description: 强制执行测试驱动开发工作流。先搭建接口，生成测试，然后实现最小代码以通过测试。确保 80%+ 覆盖率。
---
...
1. **搭建接口** - 首先定义类型/接口
2. **先生成测试** - 编写失败的测试（红灯）
3. **实现最小代码** - 只写足够通过测试的代码（绿灯）
4. **重构** - 在保持测试绿灯的同时改进代码（重构）
5. **验证覆盖率** - 确保 80%+ 测试覆盖率
